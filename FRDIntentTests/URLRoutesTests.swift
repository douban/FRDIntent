...
class URLRoutesTests: XCTestCase {
  ...
  func testURLRoutes() {
    ...
    router.register(url: URL(string: "/user/:userId")!) { (params: [String: AnyObject]) in
    ...
    _ = router.route(url: URL(string: "/user/12")!)

    router.register(url: URL(string: "/story/:storyId")!) { (params: [String: AnyObject]) in
    ...
    _ = router.route(url: URL(string: "/story/21/?key1=value1&key2=value2#ref")!)
